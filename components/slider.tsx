...
export interface SliderColors {
  /** Track background gradient (top → bottom) */
  trackGradient?: string;
  /** Channel (unfilled groove) background */
  channelBackground?: string;
  /** Filled portion gradient (left → right) */
  fillGradient?: string;
  /** Glow color behind the fill / handle while dragging, as an rgba string */
  glowColor?: string;
  /** Handle gradient */
  handleGradient?: string;
  /** Small dot inside the handle */
  handleDotColor?: string;
  /** Readout tooltip background */
  readoutBackground?: string;
  /** Readout tooltip border */
  readoutBorder?: string;
  /** Readout number text color */
  readoutTextColor?: string;
  /** Readout unit/suffix text color */
  readoutSuffixColor?: string;
  /** Page/section background, only used by the standalone demo export */
  pageBackground?: string;
  /** Label text color below the track */
  labelColor?: string;
}

const defaultColors: Required<SliderColors> = {
...
export interface SliderProps {
  /** Current value. Required — this is a controlled component. */
  value: number;
  /** Called with the next value whenever it changes via drag, touch, or keys. */
  onChange: (value: number) => void;
  /** Called once when a drag/touch/key interaction completes. Useful for committing to state/server only on release. */
  onChangeCommitted?: (value: number) => void;
  min?: number;
  max?: number;
  /** Increment applied per keyboard arrow press / Page Up/Down (10x step) */
  step?: number;
  disabled?: boolean;
  /** Visual width of the track in px */
  trackWidth?: number;
  /** Suffix shown after the number in the floating readout, e.g. "%", "px", "" */
  unit?: string;
  /** Number of digits to pad the readout to, e.g. 2 -> "07". Set 0 to disable padding. */
  padDigits?: number;
  /** Text shown below the track. Pass null to hide entirely. */
  label?: React.ReactNode;
  /** Always show the floating readout, not just while dragging */
  showReadoutAlways?: boolean;
  /** Hide the floating readout entirely */
  hideReadout?: boolean;
  /** Format the displayed value yourself, e.g. (v) => `$${v}` */
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                transition: drag ? "none" : "box-shadow 200ms ease, width 100ms ease",
              }}
            />
          </div>
