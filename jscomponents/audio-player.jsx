...
function IconButton({ label, size = "md", className, children, ...props }) {
  return (
    <button
      type="button"
      aria-label={label}
 ...
 * @property {string} [title] - Track title shown above the seek bar
 * @property {string} [subtitle] - Artist or subtitle shown below the title
 * @property {string} [imageSrc] - Cover art image URL
 * @property {string} [imageAlt] - Alt text for the cover image
 ...
 * @property {number} [skipSeconds] - Seconds the ±skip buttons jump. Defaults to 2.
 * @property {boolean} [hideVolume] - Hide the volume control
 * @property {boolean} [hideSkip] - Hide the ±skip buttons
 * @property {number|string} [width] - Explicit width — defaults to 100% of the container
 * @property {"default"|"elevated"|"ghost"} [variant]
 * @property {"sm"|"md"|"lg"} [size]
 * @property {(currentTime: number, duration: number) => void} [onTimeUpdate]
 * @property {() => void} [onEnded]
 */

/**
 * Compact pill audio player with seek, skip, volume + mute, and cover image slot.
 *
 * @type {React.ForwardRefExoticComponent<AudioPlayerProps & React.HTMLAttributes<HTMLDivElement> & React.RefAttributes<HTMLDivElement>>}
 */
export const AudioPlayer = React.forwardRef(function AudioPlayer(
  {
    src,
    title,
    subtitle,
  ...
  function ratioFromTrack(clientX, trackRef) {
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <span
            className={cn(
              "shrink-0 tabular-nums text-zinc-400 dark:text-zinc-500",
              timeSizes[resolvedSize],
            )}
          >
        ...
        >
          {playing ? (
            ...
            <Play
              className={cn(
                playIconSizes[resolvedSize],
                "fill-current stroke-none translate-x-px",
              )}
            />
          )}
        </button>
